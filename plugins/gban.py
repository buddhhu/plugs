...
@plus_ub(pattern="gban(?: |$)(.*)", from_users=sudo)
...
@plus_ub(pattern="ungban(?: |$)(.*)", from_users=sudo)
async def gspider(rk):
   lazy = rk ; sender = await lazy.get_sender() ; me = await lazy.client.get_me()
   if not sender.id == me.id:
        rkp = await lazy.reply("`processing...`")
...
\n**Usage**: Global unban user from all groups, channels , remove user from gban watch.\
"
})
