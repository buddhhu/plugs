...
@plus_ub(pattern="lock( (?P<target>\S+)|$)", from_users=sudo)
async def _(event):
...
@plus_ub(pattern="unlock ?(.*)", from_users=sudo)
async def _(event):
    if event.fwd_from:
        return
...
@plus_ub(pattern="listnabledlocks", from_users=sudo)
async def _(event):
    if event.fwd_from:
        return
    res = ""
            ...
            for user_id in added_users:
                ...
                if user_obj.bot:
                    ...
                    try:
                        ...
                        break
            if Var.BOTLOG_CHATID is not None and is_ban_able:
