...
@plus_ub(pattern="update ?(.*)", from_users=sudo)
async def upstream(ups):
    "For .update command, check if the bot is up to date, update if specified"
    pagal = await ups.reply("`Checking for updates, please wait....`")
    ...
    if conf != "now" and not force_updateme:
        ...
        if len(changelog_str) > 4096:
            await pagal.edit("`Changelog is too big, view the file to see it.`")
            ...
            await pagal.edit(changelog_str)
        await ups.respond(f'Do [`{xxx}update now`] to update')
        return
    ...
    if force_updateme:
        ...
        await pagal.edit('`Updating userbot, please wait....`')
    ...
    if Var.HEROKU_API_KEY is not None:
        ...
        if not Var.HEROKU_APP_NAME:
            await pagal.edit('`Please set up the HEROKU_APP_NAME variable to be able to update userbot.`')
            repo.__del__()
            return
        ...
        if heroku_app is None:
            ...
            repo.__del__()
            return
        await pagal.edit('`Userbot dyno build in progress, please wait for it to complete.`'
                       )
        ups_rem.fetch(ac_br)
        ...
        try:
            ...
            repo.__del__()
            return
        await pagal.edit('`Successfully Updated!\n'
