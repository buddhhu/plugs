...
@plus_ub(pattern=r"dl(?: |$)(.*)", from_users=sudo)
...
@plus_ub(pattern=r"uploadir (.*)", from_users=sudo)
...
@plus_ub(pattern=r"ul (.*)", from_users=sudo)
async def upload(u_event):
    """ For .upload command, allows you to upload a file from the userbot's server """
    b = await u_event.reply("Processing ...")
    input_str = u_event.pattern_match.group(1)
    if input_str in ("userbot.session", "config.env"):
        await b.edit("`That's a dangerous operation! Not Permitted!`")
        return
    if os.path.exists(input_str):
        c_time = time.time()
...
@plus_ub(pattern=r"uploadas(stream|vn|all) (.*)", from_users=sudo)
    ...
    if os.path.exists(file_name):
        ...
        try:
            if supports_streaming:
                ...
                return
            os.remove(thumb)
            await r.edit("Uploaded successfully !!")
        except FileNotFoundError as err:
            await r.edit(str(err))
    else:
