...
@plus_ub(pattern="nfc (.*)", from_users=sudo)  # pylint:disable=E0602
async def _(event):
    if event.fwd_from:
        return
    ...
    if reply_message is None:
        ...
        return
    a = await event.reply("trying to download media file, to my local")
    try:
        start = datetime.now()
        #c_time = time.ctime()
        ...
        await a.edit("Downloaded to `{}` in {} seconds.".format(downloaded_file_name, ms))
        ...
        if input_str == "voice":
            ...
            new_required_file_name = Var.TEMP_DOWNLOAD_DIRECTORY + "/" + new_required_file_caption
            command_to_run = [
                "ffmpeg",
                "-i",
        ...
        if os.path.exists(new_required_file_name):
            ...
            await a.edit(f"converted in {ms_two} seconds")
