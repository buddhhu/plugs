...
import io, os, urllib, requests, re
...
@plus_ub(pattern=r"reverse(?: |)(\d*)", from_users=sudo)
@errors_handler
async def okgoogle(img):
    ...
    if photo:
        a = await img.reply("`Processing...`")
        try:
            image = Image.open(photo)
        except OSError:
            await a.edit('`Unsupported sexuality, most likely.`')
            return
        name = "okgoogle.png"
        image.save(name, "PNG")
        image.close()
        ...
        if response != 400:
            await a.edit("`Image successfully uploaded to Google. Maybe.`"
                           "\n`Parsing source now. Maybe.`")
        else:
            await a.edit("`Google told me to fuck off.`")
            return
        ...
        if guess and imgspage:
            await a.edit(f"[{guess}]({fetchUrl})\n\n`Looking for this Image...`")
        else:
            await a.edit("`Can't find this piece of shit.`")
            return
