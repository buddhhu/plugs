...
@plus_ub(pattern="telegraph (media|text) ?(.*)", from_users=sudo)
async def _(event):
    if event.fwd_from:
        return
    ...
    if event.reply_to_msg_id:
        ...
        if input_str == "media":
            ...
            try:
                ...
                await a.edit("ERROR: " + str(exc))
                os.remove(downloaded_file_name)
            else:
                end = datetime.now()
                ms_two = (end - start).seconds
                os.remove(downloaded_file_name)
                await a.edit("Uploaded [here](https://telegra.ph{}) in {} seconds.".format(media_urls[0], (ms + ms_two)), link_preview=True)
        elif input_str == "text":
            ...
            ms = (end - start).seconds
            await event.reply("Pasted [here](https://telegra.ph/{}) in {} seconds.".format(response["path"], ms), link_preview=True)
    else:
