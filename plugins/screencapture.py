...
@plus_ub(pattern="screencapture (.*)", from_users=sudo)
async def _(event):
    if event.fwd_from:
        return
    if Var.SCREEN_SHOT_LAYER_ACCESS_KEY is None:
    ...
    response_api = requests.get(sample_url.format(
        Var.SCREEN_SHOT_LAYER_ACCESS_KEY,
        input_str,
        "1",
        "2560x1440",
        "PNG",
    ...
    if "image" in contentType:
        ...
        await a.edit(response_api.text)
