...
@plus_ub(pattern="unbanall ?(.*)", from_users=sudo)
async def _(event):
    if event.fwd_from:
        return
    ...
    if input_str:
        ...
        if event.is_private:
            return False
        a = await event.reply("Searching Participant Lists.")
        p = 0
        async for i in event.client.iter_participants(event.chat_id, filter=ChannelParticipantsKicked, aggressive=True):
            ...
            try:
                ...
                p += 1
        await a.edit("{}: {} unbanned".format(event.chat_id, p))


@plus_ub(pattern="ikuck ?(.*)", from_users=sudo)
async def _(event):
    if event.fwd_from:
        return
    if event.is_private:
...
Bots: {}
None: {}**"""
        await a.edit(required_string.format(c, p, d, y, m, w, o, q, r, b, n))
        await asyncio.sleep(5)
    await a.edit("""**Total :-** `{}` users
**d** :- Deleted Accounts: `{}`
**y** :- UserStatusEmpty: `{}`
...
**b** :- Bots: `{}`
**n** :- None: `{}`""".format(p, d, y, m, w, o, q, r, b, n))
