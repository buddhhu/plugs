...
import asyncio, datetime, math, os, requests, zipfile
...
@plus_ub(pattern="kang ?(.*)", from_users=sudo)
async def _(event):
    if event.fwd_from:
        return
    if not event.is_reply:
        await event.reply("Reply to a photo to add to my personal sticker pack.**( ఠ ͟ʖ ఠ)")
        return
    ...
    file_ext_ns_ion = "plus.png"
    file = await event.client.download_file(reply_message.media)
    uploaded_sticker = None
    if is_a_s:
        file_ext_ns_ion = "AnimatedSticker.tgs"
        uploaded_sticker = await event.client.upload_file(file, file_name=file_ext_ns_ion)
        packname = f"{user.first_name}'s Animated {pack}"
        if userid == 667805879:
            packshortname = "Buddhhu_Animated"
        else:
            packshortname = f"{user.first_name}'s_animated_{pack}" # format: Uni_Borg_userid
    elif not is_message_image(reply_message):
        await a.edit("Invalid message type")
        return
    else:
        with BytesIO(file) as mem_file, BytesIO() as sticker:
            resize_image(mem_file, sticker)
            sticker.seek(0)
            uploaded_sticker = await event.client.upload_file(sticker, file_name=file_ext_ns_ion)
    ...
    async with event.client.conversation("@Stickers") as bot_conv:
        ...
        if not await stickerset_exists(bot_conv, packshortname):
            await a.edit("`Brewing a new pack! ヽ(´▽｀)ノ`")
            ...
            if "Yay!" not in response.text:
                await a.edit(f"**FAILED**! @Stickers replied: {response.text}")
                return
            response = await silently_send_message(bot_conv, packname)
            if not response.text.startswith("Alright!"):
                if "unacceptable" in response.text:
                    packname = f"{user.id}'s @plus Vol.{pack}"
                    response = await silently_send_message(bot_conv, packname)
                else:
            ...
            if response.text == FILLED_UP_DADDY:
                while response.text == FILLED_UP_DADDY:
                    ...
                    if not await stickerset_exists(bot_conv, packshortname):
                        await a.edit("**Pack No. **" + str(prevv) + "** full! Making a new Pack, Vol. **" + str(pack))
                        if is_a_s:
                            response = await silently_send_message(bot_conv, "/newanimated")
                        else:
                            response = await silently_send_message(bot_conv, "/newpack")
                        ...
                        if not response.text.startswith("Alright!"):
                            if "unacceptable" in response.text:
                                packname = f"{user.id}'s @plusub Vol.{pack}"
                                response = await silently_send_message(bot_conv, packname)
                            else:
                                await a.edit(f"**FAILED**! @Stickers replied: {response.text}")
                        w = await bot_conv.send_file(
                        ...
                        await a.edit("Pack No. " + str(prevv) + " full! Switching to Vol. " + str(pack))
                        await silently_send_message(bot_conv, "/addsticker")
                        await silently_send_message(bot_conv, packshortname)                                                                            
                        await bot_conv.send_file(
                            file=uploaded_sticker,
    ...
    await a.edit(f"**Kanged!** `This sticker has been stolen to` [this place](t.me/addstickers/{packshortname}), pack{pack}"
                     f" `by` {DEFAULTUSER}\n**ᕦ(ò_ó)ᕤ**")


@plus_ub(pattern="packinfo", from_users=sudo)
async def _(event):
    if event.fwd_from:
        return
    if not event.is_reply:
...
@plus_ub(pattern="getsticker ?(.*)", from_users=sudo)
async def _(event):
    if event.fwd_from:
        return
    input_str = event.pattern_match.group(1)
    ...
    if event.reply_to_msg_id:
        ...
        file_caption = "Join @plusub"
        if is_a_s:
            file_ext_ns_ion = "tgs"
            file_caption = "Forward the ZIP file to @AnimatedStickersRoBot to get lottIE JSON containing the vector information."
        sticker_set = await event.client(GetStickerSetRequest(sticker_attrib.stickerset))
        pack_file = os.path.join(Var.TEMP_DOWNLOAD_DIRECTORY, sticker_set.set.short_name, "pack.txt")
        ...
        pending_tasks = [
            asyncio.ensure_future(
                download(document, emojis, Var.TEMP_DOWNLOAD_DIRECTORY + sticker_set.set.short_name, f"{i:03d}.{file_ext_ns_ion}")
            ) for i, document in enumerate(sticker_set.documents)
        ]
        c = await event.reply(f"Downloading {sticker_set.set.count} sticker(s) to .{Var.TEMP_DOWNLOAD_DIRECTORY}{sticker_set.set.short_name}...")
        ...
        while 1:
            ...
            if not pending_tasks:
                break
        await c.edit("Downloading to my local completed")
        # https://gist.github.com/udf/e4e3dbb2e831c8b580d8fddd312714f7
        directory_name = Var.TEMP_DOWNLOAD_DIRECTORY + sticker_set.set.short_name
        ...
        try:
            ...
            pass
        await c.edit("task Completed")
        await asyncio.sleep(3)
        await event.delete()
    else:
        await c.edit("TODO: Not Implemented")
