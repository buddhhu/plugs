...
@plus_ub(pattern="ugdrive ?(.*)", from_users=sudo)
async def _(event):
    if event.fwd_from:
        return
...
@plus_ub(pattern="drivesch ?(.*)", from_users=sudo)
async def sch(event):
    if event.fwd_from:
        return
    if CLIENT_ID is None or CLIENT_SECRET is None:
        await event.reply("This module requires credentials from https://da.gd/so63O. Aborting!")
        return False    
    try:
        with open(G_DRIVE_TOKEN_FILE) as f:
            pass
        ...
        f = open(G_DRIVE_TOKEN_FILE, "r")
        token_file_data = f.read()
        await event.client.send_message(Var.BOTLOG_CHATID, "Please add Var AUTH_TOKEN_DATA with the following as the value:\n\n`" + token_file_data + "`")
        # Authorize, get file parameters, upload file and print out result URL for download
    http = authorize(G_DRIVE_TOKEN_FILE, None)    
    input_str = event.pattern_match.group(1).strip()
    a = await event.reply("Searching for {} in G-Drive.".format(input_str))
...
@plus_ub(pattern="gdrivedir ?(.*)", from_users=sudo)
async def _(event):
    if event.fwd_from:
        return
    if CLIENT_ID is None or CLIENT_SECRET is None:
    ...
    if Var.PRIVATE_GROUP_ID is None:
        await event.reply("Please set the required environment variable `BOTLOG_CHATID` for this plugin to work")
        return
    input_str = event.pattern_match.group(1)
    if os.path.isdir(input_str):
        ...
        a = await event.reply("Uploading `{}` to G-Drive...".format(input_str))
        ...
        await a.edit(f"__Successfully Uploaded Folder To G-Drive...__\n[{input_str}]({dir_link})")
    else:
        await event.reply(f"directory {input_str} does not seem to exist")
...
async def upload_file(http, file_path, file_name, mime_type, event, parent_id):
    ...
    body = {
        "title": file_name,
        "description": "Uploaded using Plus+ Userbot gDrive v1",
        "mimeType": mime_type,
    }
    if parent_id is not None:
...
@plus_ub(pattern="gfolder", from_users=sudo)
async def _(event):
    if event.fwd_from:
        return
    folder_link = "https://drive.google.com/folderview?id="+parent_id    
    await event.reply("`Here is Your G-Drive Folder link : `\n"+folder_link)
